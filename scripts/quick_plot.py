...
data = get_data_from_array(filepath_array, prepro_para, t_start, timestamps, duration=timedelta(minutes=n_minutes))

# max_strain, max_channel, min_strain, min_channel = max_min_strain_rate(data, [8000, 8400])
# print(f"Min Strain: {min_strain} at channel {min_channel}, Max Strain: {max_strain} at channel {max_channel}")

fig1 = plt.figure()
# img1 = plt.imshow(data, aspect='auto', interpolation='none', extent=(depth[first_channel],depth[last_channel-1], ((second_time_sample - 1)/fs), (first_time_sample/fs)), vmin=-bounds, vmax=bounds)   # extent=(first_channel, last_channel, ((second_time_sample - 1)/fs), (first_time_sample/fs)), 
img1 = plt.imshow(data, aspect='auto', interpolation='none', extent=(prepro_para.get('cha1'), prepro_para.get('cha2'), data.shape[0]/prepro_para.get('target_sps'), 0), vmin=np.nanpercentile(data, 1), vmax=np.nanpercentile(data, 99))
